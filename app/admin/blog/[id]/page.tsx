...
const CATEGORIES = ['Education', 'Healthcare', 'Social Welfare', 'Widows Outreach', 'Orphanage Outreach', 'General']
...
export default function BlogPostFormPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const posts = await blogApi.adminGetPosts()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNew) {
        ...
        await blogApi.updatePost(postId!, form)
      }
  ...
  if (loading) {
    return (
      <AdminLayout title={isNew ? 'New Blog Post' : 'Edit Blog Post'}>
    ...
    <AdminLayout title={isNew ? 'New Blog Post' : 'Edit Blog Post'}>
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/admin/blog" className="p-2 text-slate-400 hover:text-slate-700 dark:hover:text-gray-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
            <ArrowLeft size={20} />
          </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-6 space-y-4">
            <h2 className="font-semibold text-slate-700 dark:text-gray-300 text-sm uppercase tracking-wider">Post Details</h2>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1.5">Excerpt</label>
              <textarea
                value={excerpt}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1.5">Content</label>
              <textarea
                value={content}
                onChange={e => setContent(e.target.value)}
                rows={12}
                placeholder="Full post content..."
                className="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-gray-100 text-sm resize-y font-mono"
              />
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-6">
            <h2 className="font-semibold text-slate-700 dark:text-gray-300 text-sm uppercase tracking-wider mb-4">Cover Image</h2>
            {imagePreview ? (
              ...
              <button
                ...
                className="w-full h-40 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl flex flex-col items-center justify-center gap-2 text-slate-400 hover:text-primary hover:border-primary transition-colors"
              >
                <ImageIcon size={32} />
          ...
          <div className="flex items-center gap-3 pb-8">
            ...
            <Link href="/admin/blog" className="px-6 py-3 text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-gray-200 font-medium text-sm">
              Cancel
            </Link>
          </div>
        </form>
